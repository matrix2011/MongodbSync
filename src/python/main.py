...
logging.basicConfig(filename=g_log_file, format='%(asctime)s %(module)s %(funcName)s %(lineno)d %(message)s',
                    level=logging.INFO if not is_debug else logging.DEBUG)
logging.info("config file: %s, log file: %s" % (g_conf_file, g_log_file))
